...
import Frontend.course_registration
...
class ManageCourse:
    """Allows Admin to manage courses; add, update, delete, fetch the data, here, admin can view all the details of
    courses and have various option to perform, they can search them by different functionality, such as course_id,
    name, and also they can search data such as id, name from one date to another.
    searching can be performed even just by providing the character that the courses detail may contain and it will
    ...
    were inserted on the course tree view. to make sorting possible, bubble sort method is used. also this method
    ...
    def __init__(self, window):
        ...
        self.window.title("Course Department - College Management System")
        ...
        self.txt = "Course Management Dashboard"
        self.count = 0
        self.text = ''
        ...
        personal_frame = LabelFrame(left_view_frame, text="Course Management Options", bg="white", fg="#4f4e4d",
                                    height=465,
        ...
        self.add_course = ImageTk.PhotoImage \
            (file='images\\add_course.png')
        self.add_course_button = Button(personal_frame, image=self.add_course, relief=FLAT, borderwidth=0,
                                         activebackground="white", bg="white", cursor="hand2",command=self.click_add_course)
        self.add_course_button.place(x=8, y=100)
        ...
        self.update_course = ImageTk.PhotoImage \
            (file='images\\update_course.png')
        self.update_course_button = Button(personal_frame, image=self.update_course, relief=FLAT, borderwidth=0,
                                            activebackground="white", bg="white", cursor="hand2",
                                             command=self.tree_event_handle)
        self.update_course_button.place(x=8, y=165)
        ...
        self.delete_course = ImageTk.PhotoImage \
            (file='images\\delete_course.png')
        self.delete_course_button = Button(personal_frame, image=self.delete_course, relief=FLAT, borderwidth=0,
                                            activebackground="white", bg="white", cursor="hand2",
                                           command=self.click_delete_course)
        self.delete_course_button.place(x=8, y=230)
        ...
        self.goto_dashboard_button = Button(personal_frame, image=self.goto_dashboard, relief=FLAT, borderwidth=0,
                                            activebackground="white", bg="white", cursor="hand2",command=self.click_go_to_dashboard)
        ...
        self.course_tree = ttk.Treeview(self.tree_view_frame,
                                         columns=(
        ...
        scroll_x.config(command=self.course_tree.xview)
        scroll_y.config(command=self.course_tree.yview)

        # ==========================TreeView Heading====================
        self.course_tree.heading("COURSE ID", text="COURSE ID")
        self.course_tree.heading("COURSE NAME", text="COURSE NAME")
        self.course_tree.heading("COURSE DURATION", text="COURSE DURATION")
        self.course_tree.heading("COURSE CREDIT", text="COURSE CREDIT")
        self.course_tree.heading("REGISTRATION DATE", text="REGISTRATION DATE")
        self.course_tree["show"] = "headings"

        # ==========================TreeView Column====================
        self.course_tree.column("COURSE ID", width=50)
        self.course_tree.column("COURSE NAME", width=150)
        self.course_tree.column("COURSE DURATION", width=100)
        self.course_tree.column("COURSE CREDIT", width=100)
        self.course_tree.column("REGISTRATION DATE", width=100)
        self.course_tree.pack(fill=BOTH, expand=1)
        self.course_tree.bind("<Delete>", self.click_delete_key)
        self.course_tree.bind("<Button-3>", self.do_popup)
        self.course_tree.bind("<Double-Button-1>", self.tree_double_click)
        self.course_tree.bind("<Return>", self.tree_double_click)
        self.search_start()
    ...
    def click_view_all(self):
        ...
        try:
            ...
            for values in data:
                ...
                print(data_list)
                self.course_tree.insert('', END, values=data_list)
    ...
    def click_add_course(self):
        """returns CourseRegisterForm class when clicked go to add course button, where admin can add new courses
        to the database"""
        win = Toplevel()
        Frontend.course_registration.CourseRegisterForm(win)
        Frontend.course_registration.Clock(win)
        self.window.withdraw()
        win.deiconify()
    ...
    def tree_event_handle(self):
        try:
            ...
            tree_view_content = self.course_tree.focus()
            tree_view_items = self.course_tree.item(tree_view_content)
            # print(tree_view_items)
            tree_view_values = tree_view_items['values']
            tree_view_id = tree_view_items['values'][0]
            # print(tree_view_id)
            ManageCourse.list_of_tree.clear()
            ManageCourse.get_id.clear()
            ManageCourse.get_id.append(tree_view_id)
            for i in tree_view_values:
                ManageCourse.list_of_tree.append(i)

            ask = messagebox.askyesno("Confirm",
                                      f"Do you want to Update Student having id {tree_view_id}")
            if ask is True:
                self.click_update_course()
    ...
    def click_update_course(self):
        """calls UpdateCourseForm class when clicked on update Course button"""
        win_ = Toplevel()
        UpdateCourseForm(win_)
        self.window.withdraw()
        win_.deiconify()

    def click_delete_key(self, events):
        """delete key of keyboard events is being handled here, and click_delete_course() methods is being called"""
        self.click_delete_course()
    ...
    def click_delete_course(self):
        ...
        try:
            ...
            course_view_content = self.course_tree.focus()
            course_view_items = self.course_tree.item(course_view_content)
            ...
            if ask is True:
                ...
                self.db_connection.delete(query, (course_view_values,))
    ...
    def do_popup(self, event):
        ...
        popup.add_separator()
        popup.add_command(label="DELETE", command=self.click_delete_course)
        popup.add_separator()
        popup.add_command(label="FETCH", command=self.click_fetch_course)
    ...
    def click_fetch_course(self):
        """when chosen fetch courses from popup menu, this methods will be executed which will grab all the
        information of that course into list and calls get_fetch_course() methods when output is True"""
        try:
            obj_course_database = Model_class.course_registration.GetDatabase('use cms;')
            self.db_connection.create(obj_course_database.get_database())
    ...
    def get_fetch_course(self):
        """calls FetchCourse class when clicked on fetch option of popup menu"""
        win_ = Toplevel()
        FetchCourseForm(win_)
        self.window.withdraw()
        win_.deiconify()
            ...
            if a.isnumeric():
                try:
                    ...
                    for child in self.course_tree.get_children():
                        val = self.course_tree.item(child)["values"][0]
                    ...
                    # print(output)
                    if output:
                        messagebox.showinfo("Found", f"course having id {output} found")
        ...
        if self.search_entry.get() != '':
            if a.isalpha():
                try:
                    search_list = []
                    for child in self.course_tree.get_children():
                        val = self.course_tree.item(child)["values"][1]
                        search_list.append(val)
                    # print(search_list)
                    ...
                    # print(output)
                    if self.output:
                        messagebox.showinfo("Found", f"course having Name '{self.output}' found")

                        obj_course_database = Model_class.course_registration.GetDatabase('use cms;')
                        self.db_connection.create(obj_course_database.get_database())

                        query = "select * from course where course_name LIKE '" + str(self.output) + "%'"
                        # print(output)
                        data = self.db_connection.select(query)
                        # print(data)
                        self.course_tree.delete(*self.course_tree.get_children())
                        ...
                        query = "select * from course where course_name LIKE '%" + str(self.search_entry.get()) + "%'"
                        # print(output)
                        data = self.db_connection.select(query)
                        # print(data)
                        self.course_tree.delete(*self.course_tree.get_children())
        ...
        if self.search_in_date_entry.get() != '':
            if True:
                try:
                    ...
                    query = "select * from course where course_name LIKE '%" + str(
                        self.search_in_date_entry.get()) + "%'  AND reg_date between %s and %s OR course_duration LIKE '%" + str(
                        self.search_in_date_entry.get()) + "%' AND reg_date between %s and %s"

                    # query = "select * from course where f_name LIKE '%" + str(self.search_in_date_entry.get()) + "%' AND reg_date between %s and %s"
                    # query = "select * from course where reg_date between %s and %s"
                    # query = "select * from course where f_name LIKE '%" + str(self.search_entry.get()) + "%' and reg_date between %s and %s"
                    # print(output)
    ...
    def start_search_combo(self, events):
        ...
        if self.search_by.get() == "Course ID":
            self.search_start()
            self.search_button.configure(command=self.search_by_id_start)

        if self.search_by.get() == "Course Name":
            self.search_start()
    ...
    def search_start(self):
        """here search frame, label, combobox are written which is shown while admin views course dashboard"""
        ...
        self.search_by['values'] = ('Search by', 'Course ID', 'Course Name','Date')
    ...
    def search_date(self):
        """here search date frame, label, calender events are written which is shown while admin view course
        dashboard"""
        ...
        self.search_fdate_entry = Entry(self.search_date_frame, highlightthickness=0, relief=FLAT, bg="white", fg="#6b6a69",
                                  font=("yu gothic ui semibold", 12))
        ...
        self.search_fdate_line = Canvas(self.search_date_frame, width=100, height=1.5, bg="#bdb9b1", highlightthickness=0)
    ...
    def sort_start(self):
        ...
        self.sort_by['values'] = ('Sort by', 'Course ID', 'Course Name','Date')
    ...
    def start_sort_combo(self,events):
        ...
        if self.sort_by.get()=="Sort by" or self.sort_by.get()=="Course ID" or self.sort_by.get()=="Course Name":
            self.sort_start()
    ...
    def sort_by_start(self, events):
        ...
        if self.sort_by.get() == "Course ID" and self.sort_in.get() == "Ascending Order":
            self.sort_button.configure(command=self.sort_by_id_ascending)
            self.sort_by_id_ascending()

        if self.sort_by.get() == "Course ID" and self.sort_in.get() == "Descending Order":
            self.sort_button.configure(command=self.sort_by_id_descending)
            self.sort_by_id_descending()

        if self.sort_by.get() == "Course Name" and self.sort_in.get() == "Ascending Order":
            self.sort_button.configure(command=self.sort_by_name_ascending)
            self.sort_by_name_ascending()

        if self.sort_by.get() == "Course Name" and self.sort_in.get() == "Descending Order":
...
class UpdateCourseForm(Frontend.course_registration.CourseRegisterForm):
    """inherits CourseRegisterForm class and overrides the functionality by configuring the submit button
    from performing add course to update courses. this class get the list of data that is selected on course tree
    view and insert those data automatically into the respected fields."""
    def __init__(self, wind):
        super().__init__(wind)
        Frontend.course_registration.Clock(wind)
        a = ManageCourse.list_of_tree
        # print(a)
        try:
            self.course_name_entry.insert(0, a[1])
            self.course_duration_entry.insert(0, a[2])
            self.course_credit_entry.insert(0, a[3])
            self.submit.configure(command=self.update)
            self.txt = f" You are updating course '{a[1]}'"
    ...
    def update(self):
        ...
        try:
            ...
            obj_course_database = Model_class.course_registration.CourseRegistration(self.course_name_entry.get(),
                                                                                        ...
                                                                                        self.reg_date)
            query = "update course set course_name=%s, course_duration=%s, course_credit=%s" \
                    " where course_id=%s"
...
class FetchCourseForm(Frontend.course_registration.CourseRegisterForm):
    """inherits CourseRegisterForm and extend the functionality by fetching the data of selected course in order to let
    user add courses by grabbing their data directly into the registration form which will save time of the admin"""
    def __init__(self, wind):
        super().__init__(wind)
        Frontend.course_registration.Clock(wind)
        a = ManageCourse.list_of_tree

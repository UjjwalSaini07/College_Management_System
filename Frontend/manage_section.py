...
import Model_class.section_registration
...
import Frontend.section_registration

import os


class ManageSection:
    """Allows Admin to manage sections; add, update, delete, fetch the data, here, admin can view all the details of
    ...
    def __init__(self, window):
        ...
        self.window.title("Section Management Dashboard - College Management System")
        ...
        self.txt = "Section Management Dashboard"
        self.count = 0
        self.text = ''
        ...
        personal_frame = LabelFrame(left_view_frame, text="Section Management Options", bg="white", fg="#4f4e4d",
                                    height=465,
        ...
        self.add_section = ImageTk.PhotoImage \
            (file='images\\add_section.png')
        self.add_section_button = Button(personal_frame, image=self.add_section, relief=FLAT, borderwidth=0,
                                         activebackground="white", bg="white", cursor="hand2",
                                         command=self.click_add_section)
        self.add_section_button.place(x=8, y=100)
        ...
        self.update_section = ImageTk.PhotoImage \
            (file='images\\update_section.png')
        self.update_section_button = Button(personal_frame, image=self.update_section, relief=FLAT, borderwidth=0,
                                            activebackground="white", bg="white", cursor="hand2",
                                            command=self.tree_event_handle)
        self.update_section_button.place(x=8, y=165)
        ...
        self.delete_section = ImageTk.PhotoImage \
            (file='images\\delete_section.png')
        self.delete_section_button = Button(personal_frame, image=self.delete_section, relief=FLAT, borderwidth=0,
                                            activebackground="white", bg="white", cursor="hand2",
                                            command=self.click_delete_section)
        self.delete_section_button.place(x=8, y=230)
        ...
        self.section_tree = ttk.Treeview(self.tree_view_frame,
                                         columns=(
        ...
        scroll_x.config(command=self.section_tree.xview)
        scroll_y.config(command=self.section_tree.yview)

        # ==========================TreeView Heading====================
        self.section_tree.heading("SECTION ID", text="SECTION ID")
        self.section_tree.heading("SECTION CODE", text="SECTION CODE")
        self.section_tree.heading("SECTION NAME", text="SECTION NAME")
        self.section_tree.heading("SECTION CAPACITY", text="SECTION CAPACITY")
        self.section_tree.heading("REGISTRATION DATE", text="REGISTRATION DATE")
        self.section_tree["show"] = "headings"

        # ==========================TreeView Column====================
        self.section_tree.column("SECTION ID", width=50)
        self.section_tree.column("SECTION CODE", width=100)
        self.section_tree.column("SECTION NAME", width=150)
        self.section_tree.column("SECTION CAPACITY", width=100)
        self.section_tree.column("REGISTRATION DATE", width=100)
        self.section_tree.pack(fill=BOTH, expand=1)
        self.section_tree.bind("<Delete>", self.click_delete_key)
        self.section_tree.bind("<Button-3>", self.do_popup)
        self.section_tree.bind("<Double-Button-1>", self.tree_double_click)
        self.section_tree.bind("<Return>", self.tree_double_click)
        self.search_start()
    ...
    def click_view_all(self):
        ...
        try:
            ...
            for values in data:
                ...
                # print(data_list)
                self.section_tree.insert('', END, values=data_list)
    ...
    def click_add_section(self):
        """returns SectionRegisterForm class when clicked go to add section button, where admin can add new sections
        to the database"""
        win = Toplevel()
        Frontend.section_registration.SectionRegisterForm(win)
        Frontend.section_registration.Clock(win)
        self.window.withdraw()
        win.deiconify()
    ...
    def tree_event_handle(self):
        try:
            ...
            tree_view_content = self.section_tree.focus()
            tree_view_items = self.section_tree.item(tree_view_content)
            # print(tree_view_items)
            tree_view_values = tree_view_items['values']
            tree_view_id = tree_view_items['values'][0]
            # print(tree_view_id)
            ManageSection.get_id.clear()
            ManageSection.list_of_tree.clear()
            ManageSection.get_id.append(tree_view_id)
            for i in tree_view_values:
                ManageSection.list_of_tree.append(i)

            ask = messagebox.askyesno("Confirm",
                                      f"Do you want to Update Student having id {tree_view_id}")
            if ask is True:
                self.click_update_section()
    ...
    def click_update_section(self):
        """calls UpdateSectionForm class when clicked on update Section button"""
        win_ = Toplevel()
        UpdateSectionForm(win_)
        self.window.withdraw()
        win_.deiconify()

    def click_delete_key(self, events):
        """delete key of keyboard events is being handled here, and click_delete_section() methods is being called"""
        self.click_delete_section()

    def click_delete_section(self):
        ...
        try:
            ...
            section_view_content = self.section_tree.focus()
            section_view_items = self.section_tree.item(section_view_content)
    ...
    def do_popup(self, event):
        ...
        popup.add_separator()
        popup.add_command(label="DELETE", command=self.click_delete_section)
        popup.add_separator()
        popup.add_command(label="FETCH", command=self.click_fetch_section)
    ...
    def click_fetch_section(self):
        """when chosen fetch sections from popup menu, this methods will be executed which will grab all the
        information of that section into list and calls get_fetch_section() methods when output is True"""
        try:
    ...
    def get_fetch_section(self):
        """calls FetchSectionForm class when clicked on fetch option of popup menu"""
        win_ = Toplevel()
        FetchSectionForm(win_)
        self.window.withdraw()
        win_.deiconify()
    ...
    def search_by_id_start(self):
        ...
        if self.search_entry.get() != '':
            if a.isnumeric():
                try:
                    search_list = []
                    for child in self.section_tree.get_children():
                        val = self.section_tree.item(child)["values"][0]
                    ...
                    # print(output)
                    if output:
                        messagebox.showinfo("Found", f"section having id {output} found")
        ...
        if self.search_entry.get() != '':
            if a.isalpha():
                try:
                    search_list = []
                    for child in self.section_tree.get_children():
                        val = self.section_tree.item(child)["values"][2]
                        search_list.append(val)
                    # print(search_list)
                    ...
                    # print(output)
                    if self.output:
                        messagebox.showinfo("Found", f"section having Name '{self.output}' found")

                        obj_section_database = Model_class.section_registration.GetDatabase('use cms;')
                        self.db_connection.create(obj_section_database.get_database())

                        query = "select * from section where section_name LIKE '" + str(self.output) + "%'"
                        # print(output)
                        data = self.db_connection.select(query)
                        # print(data)
                        self.section_tree.delete(*self.section_tree.get_children())
                        ...
                        query = "select * from section where section_name LIKE '%" + str(self.search_entry.get()) + "%'"
                        # print(output)
                        data = self.db_connection.select(query)
                        # print(data)
                        self.section_tree.delete(*self.section_tree.get_children())
        ...
        if self.search_in_date_entry.get() != '':
            if True:
                try:
                    ...
                    query = "select * from section where section_name LIKE '%" + str(
                        self.search_in_date_entry.get()) + "%'  AND reg_date between %s and %s OR section_code LIKE '%" + str(
                        self.search_in_date_entry.get()) + "%' AND reg_date between %s and %s"

                    # query = "select * from section where section_name LIKE '%" + str(self.search_in_date_entry.get()) + "%' AND reg_date between %s and %s"
                    # query = "select * from section where reg_date between %s and %s"
                    # query = "select * from section where section_name LIKE '%" + str(self.search_entry.get()) + "%' and reg_date between %s and %s"
                    # print(output)
    ...
    def start_search_combo(self, events):
        ...
        if self.search_by.get() == "Section ID":
            self.search_start()
            self.search_button.configure(command=self.search_by_id_start)

        if self.search_by.get() == "Section Name":
            self.search_start()
    ...
    def search_start(self):
        """here search frame, label, combobox are written which is shown while admin views section dashboard"""
        ...
        self.search_by['values'] = ('Search by', 'Section ID', 'Section Name', 'Date')
    ...
    def search_date(self):
        """here search date frame, label, calender events are written which is shown while admin view section
        dashboard"""
    ...
    def t_pick_date(self, event):
        ...
        self.t_date_win.title('Choose Date')
    ...
    def sort_start(self):
        ...
        self.sort_by['values'] = ('Sort by', 'Section ID', 'Section Name', 'Date')
    ...
    def start_sort_combo(self, events):
        ...
        if self.sort_by.get() == "Sort by" or self.sort_by.get() == "Section ID" or self.sort_by.get() == "Section Name":
            self.sort_start()
    ...
    def s_t_pick_date(self, event):
        ...
        self.s_t_date_win.title('Choose Date')
    ...
    def sort_by_start(self, events):
        ...
        if self.sort_by.get() == "Section ID" and self.sort_in.get() == "Ascending Order":
            self.sort_button.configure(command=self.sort_by_id_ascending)
            self.sort_by_id_ascending()

        if self.sort_by.get() == "Section ID" and self.sort_in.get() == "Descending Order":
            self.sort_button.configure(command=self.sort_by_id_descending)
            self.sort_by_id_descending()

        if self.sort_by.get() == "Section Name" and self.sort_in.get() == "Ascending Order":
            self.sort_button.configure(command=self.sort_by_name_ascending)
            self.sort_by_name_ascending()

        if self.sort_by.get() == "Section Name" and self.sort_in.get() == "Descending Order":
...
class UpdateSectionForm(Frontend.section_registration.SectionRegisterForm):
    """inherits SectionRegisterForm class and overrides the functionality by configuring the submit button
    ...
    def __init__(self, wind):
        ...
        a = ManageSection.list_of_tree
        # print(a)
        try:
            self.section_code_entry.insert(0, a[1])
            self.section_name_entry.insert(0, a[2])
            self.section_capacity_entry.insert(0, a[3])
            self.submit.configure(command=self.update)
            self.txt = f" You are updating Section '{a[2]}'"
    ...
    def update(self):
        ...
        try:
            ...
            obj_section_database = Model_class.section_registration.SectionRegistration(self.section_code_entry.get(),
                                                                                        ...
                                                                                        self.reg_date)
            query = "update section set section_code=%s,section_name=%s, section_capacity=%s" \
                    " where section_id=%s"
...
class FetchSectionForm(Frontend.section_registration.SectionRegisterForm):
    """inherits SectionRegisterForm and extend the functionality by fetching the data of selected section in order to let
    user add sections by grabbing their data directly into the registration form which will save time of the admin"""
    def __init__(self, wind):
        super().__init__(wind)
        Frontend.section_registration.Clock(wind)
        a = ManageSection.list_of_tree

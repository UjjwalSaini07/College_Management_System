...
class DepartmentRegisterForm:
    """allows admin to register departments, it validates, if that department code and name already exists or not, if
    ...
    def __init__(self, wind):
        self.window = wind
        self.window.title('DEPARTMENT REGISTRATION FORM - COLLEGE MANAGEMENT SYSTEM')
        ...
        self.txt = "Department Registration Form"
        self.count = 0
        self.text = ''
        ...
        self.dept_frame = LabelFrame(self.reg_frame, text="Department Details", bg="white", fg="#4f4e4d", height=380,
                                     width=800, borderwidth=2.4,
                                     font=("yu gothic ui", 13, "bold"))
        self.dept_frame.config(highlightbackground="red")
        self.dept_frame.place(x=100, y=170)
        ...
        self.department_code_label = Label(self.dept_frame, text="Department Code ", bg="white", fg="#4f4e4d",
                                           font=("yu gothic ui", 13, "bold"))
        self.department_code_label.place(x=160, y=80)

        self.department_code_entry = Entry(self.window, highlightthickness=0, relief=FLAT, bg="white", fg="#6b6a69",
                                           font=("yu gothic ui semibold", 12))
        self.department_code_entry.place(x=450, y=307, width=295)  # trebuchet ms

        self.department_code_line = Canvas(self.window, width=295, height=1.5, bg="#bdb9b1", highlightthickness=0)
        self.department_code_line.place(x=450, y=329)

        # ========================================================================
        # ============================Department Name=======================================
        # ========================================================================

        self.department_name_label = Label(self.dept_frame, text="Department Name ", bg="white", fg="#4f4e4d",
                                           font=("yu gothic ui", 13, "bold"))
        self.department_name_label.place(x=160, y=140)

        self.department_name_entry = Entry(self.window, highlightthickness=0, relief=FLAT, bg="white", fg="#6b6a69",
                                           font=("yu gothic ui semibold", 12))
        self.department_name_entry.place(x=450, y=367, width=295)  # trebuchet ms

        self.department_name_line = Canvas(self.window, width=295, height=1.5, bg="#bdb9b1", highlightthickness=0)
        self.department_name_line.place(x=450, y=389)
        ...
        self.submit = Button(self.dept_frame, image=self.submit_img,
        ...
        self.clear_button = Button(self.dept_frame, image=self.clear_img,
                                   font=("yu gothic ui", 13, "bold"), relief=FLAT, activebackground="white"
        ...
        self.back_button = Button(self.dept_frame, image=self.back_img,
                                  font=("yu gothic ui", 13, "bold"), relief=FLAT, activebackground="white"
        ...
        self.exit_button = Button(self.dept_frame, image=self.exit_img,
                                  font=("yu gothic ui", 13, "bold"), relief=FLAT, activebackground="white"
                                  , borderwidth=0, background="white", cursor="hand2", command=self.exit)
        self.exit_button.place(x=570, y=270)

    def click_clear_button(self):
        self.department_code_entry.delete(0, END)
        self.department_name_entry.delete(0, END)

    def click_back_button(self):
        """returns ManageDepartment class if clicked on back button"""
        win = Toplevel()
    ...
    def validation(self):
        ...
        if self.department_code_entry.get() in self.code_list:
            messagebox.showerror("Already Exists", f"{self.department_code_entry.get()} Department code Already Exists")
            # print(self.department_code_entry.get())
        elif self.department_name_entry.get() in self.name_list:
            messagebox.showerror("Already Exists", f"{self.department_name_entry.get()} Department name Already Exists")

        elif self.department_code_entry.get() == "" or self.department_name_entry.get() == "":
    ...
    def click_submit(self):
        ...
        try:
            ...
            obj_department_database = Model_class.department_registration.DepartmentRegistration(
                self.department_code_entry.get(), self.department_name_entry.get(), self.reg_date)

            query = 'insert into department (department_code,department_name,reg_date) values (%s,%s,%s);'
